...
@(private = "file") BOX_X :: render.VIEW_X + 16
@(private = "file") BOX_W :: render.PLAY_W - 32
...
reward_cell :: proc(count, k: i32) -> rl.Rectangle {
	...
	cell := min(f32(CELL_MAX),
		(f32(render.PLAY_W - 2 * GRID_SIDE) - f32(cols - 1) * CELL_GAP) / f32(cols),
...
reward_draw :: proc(r: ^render.Renderer, s: ^sim.State, names: ^ui.Player_Names) {
	...
	ui.menu_draw_text(r, "CHOOSE AN UPGRADE", CX, TITLE_Y, header, .Centre)
	ui.menu_draw_text(r, "MOVE TO PICK -- FIRE AIR LOCKS IN -- FIRE GROUND CHANGES", CX, HINT_Y, dim, .Centre)

	for k in 0 ..< rw.count {
		cell := reward_cell(rw.count, k)
		rl.DrawRectangleRec(window_rect(cell), {16, 20, 28, 230})
		rl.DrawRectangleLinesEx(window_rect(cell), render.WINDOW_SCALE, {70, 80, 92, 255})
		...
		for i in 0 ..< sim.MAX_PLAYERS {
			...
			rl.DrawRectangleLinesEx(window_rect(b), BORDER_WIDTH * render.WINDOW_SCALE, reward_accent(r.accents[i].hue, rw.locked[i]))
			ring += 1
		}
	}

	rl.DrawRectangleRec(window_rect({BOX_X, BOX_Y, BOX_W, BOX_H}), {10, 12, 18, 235})
	rl.DrawRectangleLinesEx(window_rect({BOX_X, BOX_Y, BOX_W, BOX_H}), render.WINDOW_SCALE, {90, 100, 112, 255})
	y := i32(BOX_Y + BOX_PAD)
...
reward_draw_icon :: proc(r: ^render.Renderer, pa: passives.Passive, cell: rl.Rectangle) {
	...
	room := (cell.width - 12) * render.WINDOW_SCALE
