...
sim_is_deterministic :: proc(t: ^testing.T) {
	session := sim.Session{seed = 0xDEADBEEF, level_id = 3, game_type = .Co_Op}
...
different_seeds_diverge :: proc(t: ^testing.T) {
	s1, s2: sim.State
	sim.init(&s1, sim.Session{seed = 1, level_id = 1, game_type = .Single})
	sim.init(&s2, sim.Session{seed = 2, level_id = 1, game_type = .Single})
...
coop_activates_two_players :: proc(t: ^testing.T) {
	s: sim.State
	sim.init(&s, sim.Session{seed = 7, level_id = 1, game_type = .Co_Op})
	testing.expect(t, s.players[0].active && s.players[1].active, "co-op needs both players")

	solo: sim.State
	sim.init(&solo, sim.Session{seed = 7, level_id = 1, game_type = .Single})
...
film_replay_matches_live_run :: proc(t: ^testing.T) {
	...
	film := sim.Film{
		session = sim.Session{seed = 555, level_id = 2, game_type = .Single},
		frames  = frames,
	}
