...
collide_entities :: proc(s: ^State, e, o: Entity, time: i32) {
	...
	if state_of(s, e).pass_hits_to_owner && ref_valid(s, e.owner) {
		entity_hit(s, entity_at(s, e.owner.index), shot_damage(s, o, ou.damage), o.owner_player, time)
		hit_owner = true
	}
	if !hit_owner {
		entity_hit(s, e, shot_damage(s, o, ou.damage), o.owner_player, time)
	}
	hit_owner = false
	if state_of(s, o).pass_hits_to_owner && ref_valid(s, e.owner) {
		entity_hit(s, entity_at(s, e.owner.index), shot_damage(s, e, eu.damage), e.owner_player, time)
		hit_owner = true
	}
	if !hit_owner {
		entity_hit(s, o, shot_damage(s, e, eu.damage), e.owner_player, time)
	}
}
